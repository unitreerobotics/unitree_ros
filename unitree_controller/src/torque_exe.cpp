...
{
    ros::init(argc, argv, "unitree_gazebo_torque_exe");
    ...
    rosbag::Bag bag;
    bag.open("/home/bmtsz/test_out/torque_joint.bag", rosbag::bagmode::Write);

    std_msgs::Float32 joint_tau;

    while (ros::ok()){

        // Own code goes here
        motiontime++;
        printf("FL_1 position: %f\n",  lowState.motorState[FL_1].q);
        ...
        if( motiontime >= 500){
            torque = (0 - lowState.motorState[FL_1].q)*10.0f + (0 - lowState.motorState[FL_1].dq)*1.0f;
            if(torque > 5.0f) torque = 5.0f;
            if(torque < -5.0f) torque = -5.0f;

            lowCmd.motorCmd[FL_1].q = PosStopF;
            lowCmd.motorCmd[FL_1].dq = VelStopF;
            lowCmd.motorCmd[FL_1].Kp = 0;
            lowCmd.motorCmd[FL_1].Kd = 0;
            lowCmd.motorCmd[FL_1].tau = torque;
        }   

        elapsed_Dur_time = ros::Time::now() - begin_time;
        ros::Time elapsed_time(elapsed_Dur_time.toSec());
        joint_tau.data = lowState.motorState[FL_1].tauEst;
        bag.write("/a1_gazebo/FL_thigh_controller/state", elapsed_time, joint_tau);      
