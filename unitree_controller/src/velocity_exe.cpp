...
{
    ros::init(argc, argv, "unitree_gazebo_velocity_exe");
    ...
    rosbag::Bag bag;
    bag.open("/home/bmtsz/test_out/velocity_joint.bag", rosbag::bagmode::Write);
    ...
    while (ros::ok()){
        ...
        if( motiontime >= 500){
            ...
            lowCmd.motorCmd[FR_2].dq = speed;
            lowCmd.motorCmd[FR_2].Kp = 0;
            // lowCmd.motorCmd[FL_2].Kd = Kv;
        ...
        joint_vel.data = lowState.motorState[FR_2].dq;
        bag.write("/a1_gazebo/FR_calf_controller/state", elapsed_time, joint_vel);                   
